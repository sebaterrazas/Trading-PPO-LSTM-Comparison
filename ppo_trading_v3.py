#!/usr/bin/env python3
"""
MULTI-ASSET PPO WITH TRANSFORMER - BEST OF BOTH WORLDS
- Multi-asset trading: BTC, S&P 500, US Bonds (from ppo_trading_v2.py)
- Same train/val/test splits as ppo_trading.py (2010-2016/2017-2018/2019-2024)
- Transformer architecture for advanced pattern recognition (from ppo_trading.py)
...
# ================================
# TRANSFORMER CLASSES FROM PPO_TRADING.PY
# ================================
...
class TransformerFeaturesExtractor(nn.Module):
    ...
    def __init__(self, observation_space: gym.Space, features_dim: int = 128, nhead: int = 4, 
        ...
        # Input projection
        self.input_dim = observation_space.shape[0]  # Multi-asset features
...
# ================================
# MULTI-ASSET FEATURES FROM PPO_TRADING_V2.PY
# ================================
...
# ================================
# MULTI-ASSET ENVIRONMENT FROM PPO_TRADING_V2.PY
# ================================

class TransformerMultiAssetEnv(gym.Env):
    """Multi-asset environment with Transformer-ready observations"""
    ...
    def step(self, action):
        ...
        # Enhanced reward function (Sharpe-like) from ppo_trading.py
        ...
        # Add diversification bonus from v2
        ...
        if total_asset_value > 0:
            ...
            diversification_bonus = max(0, 1 - allocation_distance * 2)  # Smaller bonus than v2
...
# ================================
# DATA LOADING AND TESTING FUNCTIONS
# ================================
...
def test_transformer_model(model, env):
    """Test the model"""
    obs, _ = env.reset()
...
def main():
    print("🚀 MULTI-ASSET PPO WITH TRANSFORMER - BEST OF BOTH WORLDS")
    print("=" * 80)
    print("✅ Multi-asset trading: BTC, S&P 500, US Bonds")
    print("✅ Transformer architecture for pattern recognition")
    print("✅ Advanced technical indicators & cross-asset correlations")
    print("✅ Robust feature handling (no NaN)")
    print("✅ Sharpe-like reward + diversification bonus")
    print("✅ SAME DATES AS PPO_TRADING.PY (2010-2024)")
    print("=" * 80)
    
    # Load data
    result = load_transformer_data()
    if result is None:
    ...
    # Create environment
    env = TransformerMultiAssetEnv(train_data)
    env = DummyVecEnv([lambda: env])
    
    # Policy kwargs for Transformer (from ppo_trading.py)
    policy_kwargs = {
    ...
    print(f"\n🧠 MODEL ARCHITECTURE:")
    print(f"   🔧 Feature Extractor: Transformer (128 dim, 4 heads, 3 layers)")
    print(f"   🎯 Action Space: MultiDiscrete([5] * 3) - 5 actions per asset")
    print(f"   📊 Observation Space: {env.envs[0].observation_space.shape[0]} features")
    ...
    val_env = TransformerMultiAssetEnv(val_data)
    val_results = test_transformer_model(model, val_env)
    ...
    test_env = TransformerMultiAssetEnv(test_data)
    results = test_transformer_model(model, test_env)
    ...
    print(f"\n✅ Multi-asset Transformer PPO training complete!")
    print(f"🎯 Combines BEST of both worlds:")
    print(f"   🔧 Multi-asset diversification (from ppo_trading_v2.py)")
    print(f"   🧠 Transformer pattern recognition (from ppo_trading.py)")
    print(f"📊 Train: 2010-2016 | Val: 2017-2018 | Test: 2019-2024")
    print(f"💾 Model saved as 'trained_models/multi_asset_transformer_ppo'")
