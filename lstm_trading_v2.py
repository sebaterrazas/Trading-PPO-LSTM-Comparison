...
def augment_trading_data(features, targets, noise_factor=0.02):
    ...
    # Add noisy versions
    for _ in range(2):  # 2 augmented versions
...
def main():
    print("🚀 MULTI-ASSET LSTM TRADING SYSTEM")
    print("=" * 70)
    print("✅ Multi-asset trading: BTC, S&P 500, US Bonds")
    print("✅ LSTM-based model (instead of PPO)")
    ...
    sequence_length = 10  # Reduced for more training data
    ...
    model = LSTMTradingModel(input_size=input_size)  # Use default parameters
    model.to(device)
    ...
    class_weights = torch.ones(5, device=device)  # Equal weights for now
    ...
    torch.save({
        ...
        'scaler': scaler
    }, "trained_models/lstm_multi_asset.pth")
    ...
    print(f"💾 Model saved as 'trained_models/lstm_multi_asset.pth'")
